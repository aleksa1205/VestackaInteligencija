...
class RubberBandUtils:
    ...
    @staticmethod
    def draw_rubber_band(surface, start_pos, end_pos, stub_radius, color, draw_circles = True):
        ...
        if draw_circles:
            pygame.draw.circle(surface, color, start_pos, stub_radius)
            pygame.draw.circle(surface, color, end_pos, stub_radius)

    @staticmethod
    def draw_rubber_bands(surface, paths, board, stub_radius, color):
        for path in paths:
            RubberBandUtils.draw_rubber_band(surface, coordinates_to_pixel(board, path[0]), coordinates_to_pixel(board, path[3]), stub_radius, color, True)